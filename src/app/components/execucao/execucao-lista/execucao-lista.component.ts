...
export class ExecucaoListaComponent implements OnInit, OnDestroy {
  ...
  constructor(private execucaoService: ExecucaoService, private activatedRoute: ActivatedRoute, private router: Router) {
  }
  ...
  pararExecucao(execucao: Execucao): void {
    if (confirm('Deseja parar: ' + execucao.executavel.titulo + '?')) {
